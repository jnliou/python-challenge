...
output_file = os.path.join(output_directory, 'pypoll_final.txt')
...
with open(output_file, "w") as datafile:
    ...
    for candidate in candidates:
        vote_percentage = votes[candidate] / total_votes * 100
        datafile.write(f'{candidate}: {vote_percentage:.2f}% ({votes[candidate]})\n')
    datafile.write(f'Winner: {candidate_name}\n')
